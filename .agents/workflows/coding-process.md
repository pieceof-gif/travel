...
### Rule 0: 구현 전 표준 패턴 파악 (가장 먼저)

UI 컴포넌트 / 기능을 새로 구현할 때 **코드 한 줄 쓰기 전에** 반드시:

```
1. 업계 표준이 있는가? (Google/YouTube/Apple 등)
2. 표준 구현 방식의 핵심 구조는 무엇인가?
3. 현재 코드베이스와 어떻게 연결하는가?
```

**예시 — Skeleton Loading 구현 전 확인했어야 할 것:**
```
Q: Google 스켈레톤의 핵심 구조는?
A: 전용 sk-* 요소 + 실제 콘텐츠 숨김 + 독립 shimmer 블록
   (::before 트릭이 아님, [class*=main] 와일드카드 금지)
```

**이것이 없으면:**
- CSS `::before` 트릭 같은 비표준 방식으로 구현
- 결과물이 나오지만 표준과 다름 → 유저가 검수에서 지적 → 재작업
...
```
□ [Rule 0] 표준 패턴 파악 완료 (Google/Apple/MDN 등 레퍼런스 확인)
