...
// 주의: 직접 편집하지 마세요. extract_js.py로 재생성하세요.
      ...
      function updateColumn(col, destIdx, budgetLimit, duration) {
        ...
        const grandTotalRaw = hotelPrice + dailyTotal;  // 항공 제외 (API 정확도 한계)
        ...
        if (dpriceEl) dpriceEl.textContent = grandTotal + '만원~ (항공 별도)';

        // 4. TOTAL COST ROW
        setCell('total-' + col, grandTotal + '만원~', '숙박+현지 비용 (항공 별도)', `${days - 1}박 ${days}일 기준`);
        ...
        setCell('air-' + col, '최저가 검색', '왕복 항공권', mainAirline ? mainAirline + ' · ' + airType : airType);
        ...
        function renderHotelList(elId, hotels, tips) {
          ...
          (hotels || []).forEach(h => {
            ...
            if (typeof h === 'string') {
              ...
              priceEl.style.display = 'none';  // 가격 정확도 한계로 비표시
          ...
          if (snapParam) {
            ...
            for (var si = 0; si < Math.min(colSnaps.length, 3); si++) {
              ...
              if (parts[1]) {
                ...
                if (ae) {
                  var am = ae.querySelector('.main'); if (am) am.textContent = '최저가 검색';
                ...
                toastEl.querySelectorAll('button')[0].onclick = function() {
                  ...
                  for (var u = 0; u < Math.min(idxArr.length, 3); u++) {
                    ...
                    if (ap) {
                      ...
                      if (ael) {
                        var m = ael.querySelector('.main'); if (m) m.textContent = '최저가 검색';
        ...
        fetch('https://travel.le2jy.workers.dev/api/flights/all?depart=' + _fDepart + '&return=' + _fReturn)
          ...
          .then(function(data) {
            ...
            v1_0_9_DEST_DATA.forEach(function(dest) {
              ...
              // 실시간 가격 → cheapFlights desc 동적 반영 (비활성: 가격 비표시 정책)
              // var _livePrice = (entry.priceLabel || '').replace(/[^0-9]/g, '');
              // 가격 부분을 desc에서 제거
              if (dest.cheapFlights) {
